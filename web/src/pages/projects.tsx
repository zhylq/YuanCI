...
function ProjectList({ userID }: { userID: string }) {
  ...
  return <div className="space-y-6">
    ...
    {projects.isFetching || projects.isPending ? <Pending label="正在校验权限并读取项目…" /> : projects.isError ? <ReadError error={projects.error} retry={() => void projects.refetch()} /> : <>
      {projects.data.items.length === 0 ? <section className={panel}><h2 className="text-balance text-xl font-semibold">{search ? '没有匹配的项目' : after ? '没有更多项目' : '还没有可见项目'}</h2><p className="mt-3 text-pretty leading-7 text-slate-600">{search ? '尝试其他名称；搜索结果仅限当前授权范围。' : '可能尚未导入仓库，或管理员还未授予你项目权限。本轮不提供仓库导入，也不会自动创建演示项目。'}</p>{search || after ? <button className={`${buttonClass} mt-4`} onClick={() => navigate('', '')}>清除筛选并回到首页</button> : <Link className={`${linkClass} mt-4 inline-flex min-h-11 items-center`} to="/">返回账号首页</Link>}</section>
        : <ul aria-label="可访问项目" className="grid min-w-0 gap-4 md:grid-cols-2">{projects.data.items.map(item => <li key={item.id} className={`${panel} min-w-0`}>
          ...
          <p className="mt-3 break-all text-sm text-slate-600">默认分支：{item.default_branch}</p><p className="mt-4 text-sm text-slate-700">仓库连接待接入</p>
        </li>)}</ul>}
      <Pager after={after} next={projects.data.next_cursor} navigate={navigate} />
    </>}
    <p className="text-pretty text-sm leading-6 text-slate-600">项目已登记不代表 Git 平台已连接。GitHub App 安装授权、仓库发现与导入将在下一阶段接入。</p>
  </div>
}
...
function ProjectDetail({ userID, projectID }: { userID: string; projectID: string }) {
  ...
  return <div className="space-y-6">
    ...
    <section className={panel}><h2 className="text-balance text-xl font-semibold">仓库信息</h2><dl className="mt-4 grid gap-5 sm:grid-cols-2"><div><dt className="text-sm text-slate-600">默认分支</dt><dd className="mt-1 break-all font-medium">{item.default_branch}</dd></div><div><dt className="text-sm text-slate-600">连接状态</dt><dd className="mt-1 font-medium">仓库连接待接入</dd></div></dl><p className="mt-4 text-pretty text-sm leading-6 text-slate-600">尚未验证仓库安装授权和 Webhook。当前不提供拉取、触发构建或发布操作。</p></section>
